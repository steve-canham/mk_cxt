...
- Lookup tables, e.g. supporting categorised data points such as study and object types, study features, language codes etc. These tables can be (re)created as required within the 'lups' scvhema of the cxt database. They are not dependent on external data sources. Recreation would normally only be after any revision of the category systems in one or more tables. 
...
- Organisation data tables, with coded lists of organisations and organisation names. These are imported from the 'ror' database, and procesed further within the 'orgs' schema of the cxt DB. Tables used directly by the MDR are copied to the 'cxt' schema within the cxt database.
- Medical vocabulary data tables, with coded categorisations of medical conditions, drugs etc. These will bw imported from the 'uml' database, and procesed further within the 'umls' schema of the cxt DB. Tables used directly by the MDR are copied to the 'cxt' schema within the cxt database.
